...
an URL is specified in the command line. quvi expects each URL to be
...
Do not verify media URL after parsing it. Normally this serves two
purposes:
...
Query for I<arg> format. I<arg> can also be C<help>.
...
Dump supported websites (of HTTP category).

=item quvi --support --category-rtmp

Same but dump supported websites of RTMP category.
See also L</NOTES - PROTOCOL CATEGORIES>

=item quvi --support --category-all

Dump B<all> supported websites.
See also L</NOTES - PROTOCOL CATEGORIES>

=item quvi --support URL

Check if URL is supported. Note that most "shortened" URLs will fail
with this. The exit status will be the last exit status of the
last checked URL. See also L</EXIT STATUS>.

=item quvi --support --category-all URL
...
Query for YouTube media URL.

=item quvi YOUTUBE_URL -f best

Query for "best" (available format) YouTube media URL.

=item quvi YOUTUBE_URL -f best --xml

Same but print details in XML.
