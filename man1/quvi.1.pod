...
Check that the parsed page title matches the I<arg>.
The libquvi tests typically make use of this option.

=item B<--media-id> I<arg>

Check that the parsed media ID matches the I<arg>.
The libquvi tests typically make use of this option.

=item B<--file-length> I<length>

Check that the parsed media content length matches the I<arg>.
The libquvi tests typically make use of this option.

=item B<--file-suffix> I<arg>

Check that the parsed media file suffix matches the I<arg>.
The libquvi tests typically make use of this option.
...
Dump media details with C<--test-all>.
...
Same but run in quiet mode. Print details in XML (to stdout).

=item quvi -t youtube

Match pattern "youtube" to a built-in media URL and test it.

=item quvi YOUTUBE_URL -f best

Query YOUTUBE_URL for "best" format.
