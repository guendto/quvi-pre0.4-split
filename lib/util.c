...
regexp_capture(_quvi_t quvi,
               const char *content,
               const char *regexp, int *dst_ovector, int ovector_count, ...)
{
  ...
  re = pcre_compile(regexp, 0, &pcre_errmsg, &error_offset, 0);
  ...
  pcre_code = pcre_exec(re,
                        ...
                        dst_ovector ? ovector_count : sizeof(ovector) /
                        sizeof(int)
      );
  ...
  pcre_code = pcre_get_substring_list(content,
                                      dst_ovector ? dst_ovector : ovector,
                                      count, &results);
  ...
  while ((dst = va_arg(ap, char **)) != 0 && i < count) {
...
int is_format_supported(const char *fmt, const char *lst)
{
  ...
  re = pcre_compile(m, PCRE_CASELESS, &pcre_errmsg, &error_offset, 0);
  ...
  pcre_code = pcre_exec(re, 0, lst, strlen(lst), 0, 0, 0, 0);
